...
public class AdminPage extends javax.swing.JFrame {
    ...
    private void btnCreateUserActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCreateUserActionPerformed
        ...
        if (fullName.length() < 2) {
            JOptionPane.showMessageDialog(CreateUserDialog, "Full name must contain at least 2 characters.");
            return;
        }

        if (!fullName.matches("[a-zA-Z ]+")) {
            JOptionPane.showMessageDialog(CreateUserDialog, "Full name must contain letters and spaces only.");
            return;
        }

        if (!email.matches("^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$")) {
            JOptionPane.showMessageDialog(CreateUserDialog, "Please enter a valid email address.");
            return;
        }

        if (!phoneNumber.matches("^\\+?[0-9]{9,15}$")) {
            JOptionPane.showMessageDialog(CreateUserDialog, "Phone number must contain 9 to 15 digits.");
            return;
        }

        if (password.length() < 6) {
            JOptionPane.showMessageDialog(CreateUserDialog, "Password must contain at least 6 characters."
            );
            return;
        }
